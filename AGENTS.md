...
| `post_index_file` | Path to the pre-built metadata index (`posts/posts.index.json`) |
...
## Landing Page, Category Page, and Search (`index.php`)

`index.php` has **three rendering paths**:
...
- **`?q=keyword`** → Standalone search results page using the JSON index. Pagination applies here.
...
Listing and pagination are driven by a **build-time metadata index** (`posts/posts.index.json`) rather than scanning all `.md` files per request.

- **Generated by:** `make build-index` → `scripts/build-index.php`
- **Used by:** `Blog::getPosts()` (for listing and search) and `Blog::getPostBySlug()` (as a fallback routing mechanism to resolve isolated post slugs) — reads index, filters, paginates, returns metadata only
...
`make docker-build` runs `make build-index` automatically before `docker build`. The index is copied into the image (`.dockerignore` allows `posts/posts.index.json`) and is available at Lambda runtime without any filesystem writes.
